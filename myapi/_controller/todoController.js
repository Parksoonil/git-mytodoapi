...
const getTotal = async () => {
  ...
  try {
    const query = `SELECT COUNT(*) AS cnt FROM ${TABLE.MYTODO}`;
...
const getSelectOne = async (id) => {
  // const getTotal = async function () {
  try {
    const query = `SELECT COUNT(*) AS cnt FROM ${TABLE.MYTODO} WHERE id=?`;
    const values = [id];
...
const getList = async (req) => {
  try {
    ...
    const query = `SELECT * FROM ${TABLE.MYTODO} where title = ? order by id desc`;
    const values = [title];
...
const todoController = {
  ...
  create: async (req) => {
    ...
    try {
      const query = `INSERT INTO mytodo (title, done) VALUES (?,?)`;
  ...
  update: async (req) => {
    ...
    try {
      const query = `UPDATE ${TABLE.MYTODO} SET title =?, done=? WHERE id= ?`;
  ...
  delete: async (req) => {
    ...
    try {
      ...
      const query = `DELETE FROM ${TABLE.MYTODO} WHERE id = ?;`;
      const values = [id];
      const [rows] = await db.execute(query, values);
      if (rows.affectedRows == 1) {
        return resData(
  ...
  reset: async (req) => {
    // truncate
    try {
      const query = `TRUNCATE TABLE ${TABLE.MYTODO};`;
    ...
    const len = req.body.len || 10;
    try {
      //더미쌓기 타이틀에 1씩추가하면서 인서트하기
      let query = `INSERT INTO mytodo (title, done) VALUES `; //.values( data1 ), (data2),,,
      let arr = [];
      for (let i = 0; i < len; i++) {
